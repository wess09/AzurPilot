...
系统设置的「界面主题」提供四种选择：浅色、深色、简约与「紧凑」。紧凑不是叠加开关，而是一个独立主题：复用极简皮肤（无毛玻璃、无装饰），并收窄侧栏（232→178px）、右栏与顶栏，压缩内容区留白、面板内边距与列表行高，结构性圆角改为直角；字号、对比度与材质保持不变，因此不靠 `zoom` 或缩小正文换取空间。它自带「主题模式」与配色方案，偏好保存在当前浏览器，规则只作用于桌面宽度（≥951px）。实现见 `src/styles/compact.css`：仅由极简皮肤导入，选择器统一用 `:root[data-theme='extreme']` 提升特异性以压过各主题与断点声明；浅色与深色主题不加载该文件。
