...
class AzurLaneAutoScript:
    ...
    def _game_restart_due(self):
        """判断定时重启游戏是否到期（纯检查，无副作用）。

        调度器启动后第一次任务前不触发，避免与"跳过第一次 Restart"分支冲突。
    ...
    def _schedule_game_restart(self):
        ...
        间隔计时由 restart() 成功路径重置，这里只负责调度、不修改
        last_game_restart_time。不引入持久的"已调度"闸门，失败/跳过场景下
    ...
    def loop(self):
        ...
        while 1:
            try:
                ...
                # 到期则安排重启；不 continue，本迭代继续执行 Restart 任务，
                # 避免持久 pending 闸门在失败/跳过场景下卡住调度
                if self._game_restart_due():
                    self._schedule_game_restart()
